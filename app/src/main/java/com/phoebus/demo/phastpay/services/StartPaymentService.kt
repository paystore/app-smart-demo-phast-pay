package com.phoebus.demo.phastpay.services

import com.google.gson.Gson
import com.phoebus.demo.phastpay.data.dto.PhastErrorResponse
import com.phoebus.demo.phastpay.data.dto.PhastPayStartPaymentRequest
import com.phoebus.demo.phastpay.data.dto.PhastPayStartPaymentResponse
import com.phoebus.demo.phastpay.utils.LastTransactionState
import com.phoebus.phastpay.sdk.client.PhastPayClient
import kotlinx.coroutines.channels.awaitClose
import kotlinx.coroutines.flow.callbackFlow

class StartPaymentService {

    operator fun invoke(
        phastPayClient: PhastPayClient,
        phastPayStartPaymentRequest: PhastPayStartPaymentRequest
    ) = callbackFlow {
        val gson = Gson()
        val callback = object : PhastPayClient.ICallbackService {
            override fun onError(response: String?) {
                val responseError = gson.fromJson(response, PhastErrorResponse::class.java)
                trySend(Result.failure(Exception(responseError.errorMessage)))
                close()
            }

            override fun onSuccess(response: String?) {
                val responseObject =
                    gson.fromJson(response, PhastPayStartPaymentResponse::class.java)
                LastTransactionState.updateAppClientId(responseObject.appClientId ?: "")
                LastTransactionState.updatePaymentId(responseObject.paymentId ?: "")
                trySend(Result.success(responseObject))
                close()
            }
        }

        try {
            val request = phastPayStartPaymentRequest.toJson()
            phastPayClient.startPayment(request, callback)
        } catch (e: Exception) {
            trySend(Result.failure(e))
            close()
        }
        awaitClose {  }
    }

}